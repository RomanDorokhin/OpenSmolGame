...
export function ChatMessageItem({ message, onRetry }: ChatMessageItemProps) {
  ...
  const handleDeploy = async () => {
    ...
    setIsDeploying(true);
    setDeployStatus(isTelegram ? "Connecting to SmolGame..." : "Initializing deployment...");
    ...
    try {
      ...
      const metadata: GameMetadata = {
        title: message.content.slice(0, 30).split('\n')[0] || "Smol Game Prototype",
        genre: "Web Game",
        description: "Generated by OpenSmolGame AI Architect",
      ...
      let result;
      if (isTelegram) {
        result = await deployer.deployViaSmolGameBackend(htmlCode, metadata, (msg) => {
          setDeployStatus(msg);
        });
      } else {
        // Get user info if not available for direct GitHub deploy
        ...
        const directDeployer = new GitHubGameDeployerOAuth({
          token: token || "",
          owner,
          repo: `smol-game-${Date.now()}`,
          branch: 'main',
        ...
        result = await directDeployer.deployGame(htmlCode, metadata, (msg) => {
          setDeployStatus(msg);
        });
      }

      if (result.success) {
        setDeployResult({ gameUrl: result.gameUrl, repoUrl: result.repositoryUrl });
      } else {
